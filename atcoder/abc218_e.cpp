...
inline long long kruskal() {
	sort(edges + 1, edges + 1 + m, [](auto a, auto b) {
		return a.w < b.w;
	});
	long long res = 0, full = 0;
	for(int i = 1; i <= m; i++)   full += edges[i].w;
...
int main() {
	io.read(n, m);
	iota(fa + 1, fa + 1 + n, 1);
	for(int i = 1; i <= m; i++)   io.read(edges[i].u, edges[i].v, edges[i].w);
	io.writeln(max(kruskal(), 0ll));
	//system("pause");
	return 0;
}