...
int tree[4 * 300000 + 10];
int n, k, arr[300010];
void adjust(int id, int l, int r, int pos, int val) {
	if(l == r)    return tree[id] = max(tree[id], val), void();
	int mid = (l + r) >> 1;
...
int main() {
	...
	for(int i = 1; i <= n; i++)
		adjust(1, 0, 300000, arr[i], query(1, 0, 300000, max(arr[i] - k, 0), min(arr[i] + k, 300000)) + 1);
	io.writeln(query(1, 0, 300000, 0, 300000));
	//system("pause");
	return 0;
}