...
queue<int> q;
queue<int> t;
int n, a[1010][1010], deg[1500010], ans;
bool vis[1500010];
...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j < n; j++) {
			...
			int y = i;
			if (x > y) swap(x, y);
			if (deg[hsh(x, y)] == 0 && vis[hsh(x, y)] == 0) q.push(hsh(x, y)), t.push(1), vis[hsh(x, y)] = 1;
		}
	}
	int tot = 0;
	while (!q.empty()) {
