...
pair<int, int> a[300010], b[300010];
long long res;
int n;
int main() {
	io.read(n);
	for (int i = 1; i <= n; ++i) {
		io.read(a[i].first, a[i].second);
		b[i] = a[i];
	}
	sort(a + 1, a + n + 1, [](auto i, auto j) {
		return i.first > j.first;
	});
	sort(b + 1, b + n + 1, [](auto i, auto j) {
		return i.second < j.second;
	});
	for (int i = 1; i <= n && a[i].first > b[i].second; ++i)
		res += 1LL * (a[i].first - b[i].second) * (n - i - i + 1);
	io.writeln(res);
	return 0;
}
