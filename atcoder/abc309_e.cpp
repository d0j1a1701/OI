...
int n, m, fa[300010], mark[300010], y[300010];
list<int> cld[300010];
...
int main() {
	memset(y, -1, sizeof(y));
	io.read(n, m);
	for(int i = 2; i <= n; i++)   io.read(fa[i]), cld[fa[i]].emplace_back(i);
	for(int i = 1, a, b; i <= m; i++)   io.read(a, b), y[a] = max(y[a], b);
	dfs(1, 0);
	io.writeln(count(mark + 1, mark + 1 + n, 1));
	//system("pause");
	return 0;
}