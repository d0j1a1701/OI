...
long long n, m, dist[310][310], res;
int main() {
	io.read(n, m);
	memset(dist, 0x3f, sizeof(dist));
	for(int i = 1; i <= n; i++)   dist[i][i] = 0;
	for(int i = 1; i <= m; i++) {
		io.read(edges[i].u, edges[i].v, edges[i].w);
		dist[edges[i].u][edges[i].v] = dist[edges[i].v][edges[i].u] = edges[i].w;
	}
	for(int k = 1; k <= n; k++)
		for(int i = 1; i <= n; i++)
			for(int j = 1; j <= n; j++)
	...
	for(int i = 1; i <= m; i++) {
		auto [u, v, w] = edges[i];
		for(int j = 1; j <= n; j++)
			if(u != j && v != j && dist[u][j] + dist[j][v] == dist[u][v]) {
				res++;
				break;
			}
	}
