...
int ch[500010][26], fail[500010], tot;
int len[500010], f[500010];
...
inline void find(char *s) {
	...
	for (int i = 0; i < n; i++) {
		k = ch[k][s[i] - 'a'];
		if (i - f[k] + 2 > maxr) res++, maxr = i + 1;
	}
}
int n, m;
char s[500010], buf[500010];
int main() {
	io.read(s), io.read(m), n = strlen(s);
	for (int i = 1; i <= m; i++) io.read(buf), insert(buf);
	build(), find(s);
	io.writeln(res);
	//system("pause");
	return 0;
