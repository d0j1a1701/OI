...
long long now[1000010], cnt[1000010];
bool prim[1000010];
long long n, k;
int main() {
	io.read(n, k);
	for(int i = 2; i <= 1000000; i++) prim[i] = 1;
	...
	for(long long i = 2; i <= 1000000; i++) {
		long long fst = ((n - k) / i + 1) * i;
		for(long long j = fst; j <= n; j += i) {
			while(now[j + k - n] % i == 0) {
				now[j + k - n] /= i;
				cnt[i]++;
			}
	...
	for(int i = 2; i <= 1000000; i++)
		res = res * (cnt[i] + 1) % mod;
	for(int i = 1; i <= k; i++) if(now[i] != 1) res = res * 2 % mod;
	io.writeln(res);
	return 0;
}
