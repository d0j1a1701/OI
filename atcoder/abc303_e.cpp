...
list<int> edges[200010], res;
int n, root;
void dfs(int id, int fa, int k) {
	if(k == 1)    res.emplace_back(edges[id].size());
	for(auto v : edges[id]) {
		if(v == fa)	continue;
		dfs(v, id, k % 3 + 1);
	}
...
int main() {
	...
	for(int i = 1; i <= n; i++)
		if(edges[i].size() > edges[root].size()) root = i;
	int cnt = 0;
	for(int i = 1; i <= n; i++)   cnt += edges[i].size() == 2;
