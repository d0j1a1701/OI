...
long long n, cnta, cntb, cntc, divide = 1;
char str[100010];
int main() {
	io.read(str + 1), n = strlen(str + 1);
	for(int i = 1 ; i <= n; i++) {
		if(str[i] == 'A') cnta = (cnta + divide) % mod;
		else if(str[i] == 'B') cntb = (cntb + cnta) % mod;
		else if(str[i] == 'C') cntc = (cntc + cntb) % mod;
		else {
