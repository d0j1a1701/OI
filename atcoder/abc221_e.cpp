...
long long n, arr[300010], po2[300010], tree[300010], res;
...
inline void adjust(int pos, long long val) {
	for(; pos <= 300000; pos += lowbit(pos))  tree[pos] = (tree[pos] + val) % mod;
}
inline long long query(int pos) {
	long long res = 0;
	for(; pos; pos -= lowbit(pos))  res = (res + tree[pos]) % mod;
	return res;
}
map<int, int> mp;
int id;
int main() {
	io.read(n), po2[0] = 1;
	for(int i = 1; i <= n; i++)   io.read(arr[i]), mp[arr[i]] = 1;
	for(auto &[k, v] : mp) v = ++id;
	for(int i = 1; i <= n; i++)   arr[i] = mp[arr[i]];
	for(int i = 1; i <= n; i++)   po2[i] = (po2[i - 1] << 1) % mod;
	for(int i = n; i >= 1; i--) {
		long long sum = (query(300000) - query(arr[i] - 1) + mod) % mod;
		sum = sum * qpow(po2[i - 1], mod - 2, mod) % mod;
		res = (res + sum) % mod;
		adjust(arr[i], po2[i - 2]);
	}
	io.writeln(res);
	//system("pause");
	return 0;
