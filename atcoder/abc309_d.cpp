...
int n1, n2, m, dist[300010];
list<int> edges[300010];
queue<int> q;
inline int bfs(int s) {
	fill(dist + 1, dist + 1 + n1 + n2, -1);
	...
	int res = 0;
	if(s == 1)    res = *max_element(dist + 1, dist + 1 + n1);
	else res = *max_element(dist + 1 + n1, dist + 1 + n1 + n2);
	return res;
}
int main() {
	io.read(n1, n2, m);
