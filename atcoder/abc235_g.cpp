...
long long n, X, Y, Z, fac[10000010], ifac[10000010], f[10000010], g[10000010], h[10000010];
inline auto qpow(auto b, auto p) {
	auto res = 1ll;
	for(auto i = p; i; i >>= 1, b = b * b % mod)
		if(i & 1)    res = res * b % mod;
	return res;
}
...
int main() {
	...
	long long res = 0;
	for (int i = 0; i <= n; ++i)
		res = (res + ((i & 1) ? mod - 1 : 1) * C(n, i) % mod * f[n - i] % mod * g[n - i] % mod * h[n - i] % mod) % mod;
	io.writeln(res);
	//system("pause");
	return 0;
}