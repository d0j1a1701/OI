...
pair<pair<long long, long long>, pair<long long, long long> > arr[1000010];
pair<long long, long long> t = {0, 1};
long long n, res, x, y;
int main() {
	io.read(n);
	for (int i = 1; i <= n; i++) {
	...
	for (int i = 1; i <= n; i++)
		if (t.first * arr[i].first.second <= arr[i].first.first * t.second)
			t = arr[i].second, res++;
	io.writeln(res);
	return 0;
}