...
list<pair<int, int> > edges[500010];
int n, dist[500010], pre[500010];
int dfs(int id, int fa) {
	int res = 0;
	for(auto v : edges[id]) {
		if(v.first == fa)	continue;
		dist[v.first] = dist[id] + v.second;
		int x = dfs(v.first, id) + v.second;
		if(x > res) res = x, pre[id] = v.first;
	}
	return res;
}
int a, b;
int main() {
	...
	dfs(1, 0), a = max_element(dist + 1, dist + 1 + n) - dist;
	int len = dfs(a, 0), flag = len & 1;
	len >>= 1;
	while(len--)    a = pre[a];
	if(flag)        b = pre[a];
	if(a && b && a > b)   swap(a, b);
	io.write(a);
	if(a && b)  io.write(' ').write(b);
	io.writeln();
	system("pause");
	return 0;
}