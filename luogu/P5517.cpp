...
using namespace std;
const int MOD = 1e9 + 7, inv32 = 281250002, POW = 369057345;
int t;
unsigned long long n, m, ans, now, pow1[32010], pow2[32010];
int main() {
	io.read(t);
	Mker::init();
	pow1[0] = pow2[0] = 1;
	...
	while(t--) {
		...
		m = n % (MOD - 1);
		now = (now * pow1[m / 32000] % MOD * pow2[m % 32000] % MOD + 21 + 30 * (n & 1)) % MOD;
		(now *= inv32) %= MOD;
		ans ^= now;
	}
	io.writeln(ans);
