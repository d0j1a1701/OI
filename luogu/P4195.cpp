...
inline long long bsgs(long long a, long long b, long long mod) {
	b %= mod, hsh.clear();
	long long t = sqrt(mod) + 1;
	long long pa = 1;
	for(long long i = 0; i < t; ++i)
		hsh[(long long)b * pa % mod] = i, pa = pa * a % mod;
	a = pa;
	if(!a) return b == 0 ? 1 : -1;
	pa = a;
...
inline long long exbsgs(long long a, long long b, long long mod) {
	if (b == 1 || mod == 1)return 0;
	long long g = __gcd(a, mod), k = 0, na = 1;
	while (g > 1) {
		if (b % g != 0)return -1;
		k++;
		b /= g;
	...
	long long f = bsgs(a, b * getInv(na, mod) % mod, mod);
	if (f == -1)return -1;
	return f + k;
}
long long a, b, mod;
signed main() {
	io.read(a, mod, b);
	do {
		auto res = exbsgs(a, b, mod);
		if(res == -1) io.writeln("No Solution");
		else io.writeln(res);
		io.read(a, mod, b);
	} while(a != 0 && b != 0 && mod != 0);
	return 0;
