...
int main() {
	...
	for(int i = 1, v, w, m; i <= n; i++) {
		...
		for(int j = 0; j < w; j++) {
			dq.clear();
			for(int k = j; k <= W; k += w) {
				while(dq.size() && lst[k] >= lst[dq.back()] + (k - dq.back()) / w * v) dq.pop_back();
				while(dq.size() && dq.front() < k - w * m)    dq.pop_front();
				dq.push_back(k);
				if(dq.size())   dp[k] = max(lst[k], lst[dq.front()] + (k - dq.front()) / w * v);
			}
		}
	}
	io.writeln(dp[W]);
