...
int n, l, r, res = 1e9, arr[1000010], fst[1000010], lst[1000010], nxt[1000010], pre[1000010];
set<int, greater<int> > ls_i;
int main() {
	io.read(n, l, r);
	for(int i = 1; i <= n; i++)   io.read(arr[i]);
	for(int i = 1; i <= 1000000; i++)   fst[i] = nxt[i] = n + 1;
	for(int i = 1; i <= 1000000; i++)   lst[i] = pre[i] = 0;
	for(int i = n; i >= 1; i--)   fst[arr[i]] = i;
	...
	for(int i = 1; i <= n; i++)
		if(i != lst[arr[i]])
			pre[arr[i]] = i;
	for(int i = 1; i < l; i++)
		if(i == fst[arr[i]] && nxt[arr[i]] > r)
			ls_i.insert(i);
	for(int i = r + 1; i <= n; i++) {
		if(nxt[arr[i]] <= i) {
			auto it = ls_i.find(fst[arr[i]]);
			if(it != ls_i.end())  ls_i.erase(it);
		}
		if(ls_i.empty())    break;
		if(i == lst[arr[i]] && pre[arr[i]] < *ls_i.begin())  res = min(res, i - *ls_i.begin() + 1);
	}
	io.writeln(res > n ? -1 : res);
	//system("pause");
	return 0;
}
/*
寻找一个包括 [l,r] 的最短区间 [i,j]，
使得 (i,j) 是唯一满足 s[a]=s[i],s[b]=s[j],a<b 的有序对，且 s[i]!=s[j]

不能二分！！！二分是假的！！没有单调性！！
i 必须是该种颜色的左端点，j 必须是该种颜色的右端点
下一个与 i 同色的必须比 j 大 -> j 之前只有 i 一种具备 arr[i] 颜色的
上一个与 j 同色的必须比 i 小 -> i 之后只有 j 一种具备 arr[j] 颜色的
*/