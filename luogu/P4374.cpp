...
int n, m, res[50010], dep[50010];
int fa[50010][17], Id[50010], f[50010];
list<pair<int, int> >edges[50010];
...
int main() {
	...
	for(int i = 1; i <= m; i++) {
		...
		int xy = lca(x, y);
		for(x = get(x); dep[x] > dep[xy]; x = get(fa[x][0])) {
			res[x] = v;
			f[x] = fa[x][0];
		}
		for(y = get(y); dep[y] > dep[xy]; y = get(fa[y][0])) {
			res[y] = v;
			f[y] = fa[y][0];
		}
	}
	for(int i = 1; i < n; i++) io.writeln(res[Id[i]]);
	return 0;
}
