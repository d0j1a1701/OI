...
int x, y, s, dp[3][65][65], res;
int main() {
	io.read(x, y);
	if(x == 1) {
		io.write(1).writeln(y);
		return 0;
	}
	for(int i = 2; i <= x; ++i) {
		for(int j = 0; j < (1 << y); ++j)
			for(int k = 0; k < (1 << y); ++k) {
				dp[i & 1][j][k] = 0;
				if(!(((j & (~(j >> 1))) & (k >> 2)) || ((j & (~(j << 1))) & (k << 2)) || (((k & (~(k >> 1))) & (j >> 2))) || ((k & (~(k << 1))) & (j << 2)))) { //前两行是否合法
					if(i == 2)
						dp[0][j][k] = 1;
					else
						for(int l = 0; l < (1 << y); ++l)
							if(!(((j & (~k)) & (l >> 1)) || ((j & (~k)) & (l << 1)) || ((l & (~k)) & (j >> 1)) || ((l & (~k)) & (j << 1))))
								dp[i & 1][j][k] = (dp[i & 1][j][k] + dp[(i & 1) ^ 1][k][l]) % mod;
				}
			}
	}
	for(int i = 0; i < 1 << y; ++i)
		for(int j = 0; j < 1 << y; ++j)
			res = (res + dp[x & 1][i][j]) % mod;
	io.writeln(res);
	return 0;
}
