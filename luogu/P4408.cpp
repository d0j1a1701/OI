...
long long n, fir[200010], sec[200010], dist[200010];
list<pair<int, long long> > edges[200010];
long long dfs(int id, int fa) {
	long long res = 0;
	for(auto [v, w] : edges[id]) {
		if(v == fa)	continue;
		dist[v] = dist[id] + w;
		res = max(res, dfs(v, id) + w);
	}
	return res;
}
namespace hpd {
const int maxn = 200010;
int dep[maxn], top[maxn], son[maxn], fa[maxn], sz[maxn], dfn[maxn], len;
int dfs(int u) {
...
inline long long getDistance(int a, int b) {
	return dist[a] + dist[b] - dist[hpd::lca(a, b)] * 2;
}
int a, b, m;
int main() {
	io.read(n, m);
	...
	dfs(1, 0);
	a = max_element(dist + 1, dist + 1 + n) - dist;
	dist[a] = 0;
	long long dia = dfs(a, 0);
	b = max_element(dist + 1, dist + 1 + n) - dist;
	hpd::build(1), dfs(1, 0);
	long long res = 0;
	for(int i = 1; i <= n; i++)
		res = max(res, dia + min(getDistance(a, i), getDistance(b, i)));
	io.writeln(res);
	system("pause");
	return 0;
}