...
inline int getDistance(int u, int v) {
	int l = hpd::lca(u, v);
	return hpd::dep[u] + hpd::dep[v] - 2 * hpd::dep[l];
}
int main() {
	io.read(n, m);
	...
	for(int i = 1, a, b, c, d; i <= m; i++) {
		io.read(a, b, c, d);
		int l1 = hpd::lca(a, b), l2 = hpd::lca(c, d);
		io.writeln((getDistance(a, l2) + getDistance(b, l2) == getDistance(a, b) || getDistance(c, l1) + getDistance(d, l1) == getDistance(c, d)) ? 'Y' : 'N');
	}
	//system("pause");
	return 0;
}