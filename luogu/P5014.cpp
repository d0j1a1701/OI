...
long long fac[2000010], inv[2000010];
inline long long c(long long n, long long m) {
	return fac[n] % mod * inv[m] % mod * inv[n - m] % mod;
}
long long solve(long long n) {
	long long a = sqrt(n) - 1, k;
	while(n - a * (a - 1) / 2 > a) a++;
	k = n - a * (a - 1) / 2;
	long long res = 0;
	for(int i = 0; i < k; i++) {
		int _a = a - 1 - i, _k = k - 1 - i;
		res += c(_a + _k + i, i) * (c(_a + _k, _a) - c(_a + _k, _a + 1) + mod) % mod;
		if(res >= mod) res -= mod;
	}
	return res;
}
int main() {
	...
	for(int i = 0; i <= 2000000; i++) inv[i] = qpow(fac[i], mod - 2, mod);
	int t = io.read();
	while(t--) io.writeln(solve(io.read<long long>()));
	//system("pause");
	return 0;
}