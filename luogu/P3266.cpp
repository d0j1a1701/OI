...
long long fac[3000010], inv[3000010], n, m, res;
...
inline auto c(auto x, auto y) {
	return (x < 0 || y < 0) ? 0 : 1ll * fac[x + y] * inv[x] % mod * inv[y] % mod;
}
void flip1(auto &x, auto &y) {
	swap(x, y);
	x--, y++;
}
void flip2(auto &x, auto &y) {
	swap(x, y);
	x += m + 2, y -= m + 2;
}
int main() {
	...
	for(int i = 1; i <= 3000000; i++)    inv[i] = qpow(fac[i], mod - 2, mod);
	auto x = n + m + 1, y = n;
	res = c(x, y);
	while(x >= 0 && y >= 0) {
		flip1(x, y);
		res = (res + mod - c(x, y)) % mod;
		flip2(x, y);
		res = (res + c(x, y)) % mod;
	}
	x = n + m + 1, y = n;
...
/*
你需要构造一个 n*m 的整数数组，值域为 [0,m]
要求 arr[i][j] 小于 右侧 的值和 右上 的值
求构造方案数

一个一定满足要求的构造方法是每一行从左到右填 [0,m-1]
如果想改，那就只能该行最后几个数 +1

性质 1：每一行最多存在一个下标，它与之后的数都 +1
性质 2：如果一个数右上角的数没有 +1，则它不可以 +1
性质 3：每一行 +1 下标的位置在 [k-1,m] 区间中（由性质 2 得到）

其中第一行随时可以 +1，注意分界点最多可达 m+1（全部不 +1）
