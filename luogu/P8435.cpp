...
/*
dfn[i]: 点 i 是第几个被访问到的
low[i]: 点 i 及其所有子节点经过最多一条反向边能到达的最小 dfn

对于根节点，只要有超过 1 个树边孩子就是割点

对于非根节点，如果存在一个节点 v 使得 low[v] >= dfn[i]，
则删去 i 后 v 即被孤立，因此 i 是割点
*/
int n, m, cnt, bcc_cnt, dfn[500010], low[500010];
		...
		if(!dfn[v]) {
			...
			if(low[v] >= dfn[u]) {
				...
				while(stk.size()) {
					auto[x, y] = stk.top();
					stk.pop();
					bcc[bcc_cnt].emplace(x);
					bcc[bcc_cnt].emplace(y);
					if(x == u && y == v)	break;
				}
			}
...
int main() {
	...
	for(int i = 1; i <= n; i++)
		if(!dfn[i]) {
			dfs(i, -1);
			if(edges[i].empty())	bcc[++bcc_cnt].emplace(i);
		}
	io.writeln(bcc_cnt);
	for(int i = 1; i <= bcc_cnt; i++) {
		io.writeWith(bcc[i].size());
		for(int u : bcc[i])
			io.writeWith(u);
		io.writeln();
	}
