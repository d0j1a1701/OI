...
int t, n, m, c[100010], s[100010], f[100010];
bool key[100010], vis[100010];
list<int> edges[100010], q[100010];
inline bool bfs1() {
	for(int i = 1; i <= n; i++)   key[i] = vis[i] = false;
	for(int i = 0; i <= n; i++)   q[i].clear();
	q[0].push_back(1), vis[1] = true;
	while(q[0].size()) {
		int u = q[0].front();
		q[0].pop_front();
		key[s[u]] = true;
		while(q[s[u]].size()) vis[q[s[u]].front()] = true, q[0].push_back(q[s[u]].front()), q[s[u]].pop_front();
		for(int v : edges[u])
			if(!vis[v] && key[c[v]]) {
				vis[v] = true;
				q[0].push_back(v);
...
int main() {
	...
	while(t--) {
		...
		for(int i = 1; i <= n; i++)   io.read(c[i]);
		for(int i = 1; i <= n; i++)   io.read(s[i]);
		for(int i = 1; i <= n; i++)   io.read(f[i]);
		...
		bfs1();
		for(int i = 1; i <= n; i++)   io.writeWith(vis[i], " ");
		io.writeln();
	}
	system("pause");
	return 0;
