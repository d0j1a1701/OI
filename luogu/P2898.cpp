...
inline bool check(int k) {
	partial_sort_copy(arr + 1, arr + 1 + k, cur + 1, cur + 1 + k, [](auto a, auto b) {
		return a.f > b.f;
	});
	int l = cur[1].l, r = cur[1].r;
	for(int i = 2; i <= k; i++) {
		int cnt = 0;
		if(cur[i].f == cur[i - 1].f)    cnt++;
		else continue;
		cnt += cur[i].r < l || cur[i].l > r;
		if(cnt == 2)  return false;
		l = min(cur[i].l, l), r = max(cur[i].r, r);
	}
	return true;
}
int main() {
	io.read(n, q);
	for(int i = 1; i <= q; i++)   io.read(arr[i].l, arr[i].r, arr[i].f);
	int l = 1, r = q, res = 0;
	while(l <= r) {
		int mid = (l + r) >> 1;
		io.writeln(l, r, mid);
