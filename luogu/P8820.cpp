...
int n, q, k;
namespace subtask1 {
int fa[200010][25], deep[200010];
long long pfx[200010];
void dfs(int u) {
	for(int v : edges[u])
		if(v != fa[u][0])
			deep[v] = deep[u] + 1, fa[v][0] = u, pfx[v] = pfx[u] + arr[v], dfs(v);
}
...
void main() {
	...
	for(int i = 1, u, v; i <= q; i++) {
		io.read(u, v);
		int l = lca(u, v);
		io.writeln(pfx[u] + pfx[v] - 2 * pfx[l] + arr[l]);
	}
}
}
namespace subtask2 {
