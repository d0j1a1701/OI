#include <iostream>
using namespace std;
/* d0j1a_1701 FastIO fulong long ver. 3.6 */
...
int main() {
	cin >> x >> y >> m >> n >> l;
	long long b = n - m, a = x - y, x1, y1;
	if(b < 0)   b = -b, a = -a;
	auto g = exgcd(b, l, x1, y1);
	if(a % g != 0)  io.writeln("Impossible");
	else io.writeln(((x1 * (a / g)) % (l / g) + (l / g)) % (l / g));
	system("pause");
	return 0;
}