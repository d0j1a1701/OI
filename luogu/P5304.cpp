...
long long t, n, m, k, dist[100010];
vector<pair<int, long long> > edges[100010];
bool vis[100010], a[100010], b[100010];
inline long long dijkstra() {
	fill(vis, vis + 1 + n + 1, 0);
	fill(dist, dist + 1 + n + 1, 0x3f3f3f3f3f3f3f3f);
	priority_queue<pair<long long, int>, vector<pair<long long, int> >, greater<pair<long long, int> > > q;
	q.emplace(0, 0), vis[0] = 1, dist[0] = 0;
	while(q.size()) {
		auto [_, u] = q.top();
		q.pop();
		vis[u] = 1;
		...
		if(b[u]) {
			if(dist[u] < dist[n + 1]) {
				dist[n + 1] = dist[u];
				if(vis[n + 1])    continue;
				q.emplace(dist[n + 1], n + 1), vis[n + 1] = 1;
			}
		}
	}
	return dist[n + 1];
...
int main() {
	io.read(t);
	while(t--) {
		io.read(n, m, k), fav.clear();
		for(int i = 1; i <= n; i++)	edges[i].clear();
		for(int i = 1, u, v, w; i <= m; i++) {
			io.read(u, v, w);
			edges[u].emplace_back(v, w);
		}
		for(int i = 1; i <= k; i++) fav.emplace_back(io.read());
		long long res = 1e18;
		for(int _ = 1; _ <= 40; _++) {
			shuffle(fav.begin(), fav.end(), rng);
			for(int i = 1; i <= n; i++) a[i] = b[i] = 0;
			for(int i = 0; i < k / 2; i++) a[fav[i]] = 1;
			for(int i = k / 2; i < k; i++) b[fav[i]] = 1;
