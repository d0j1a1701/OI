...
pair<long long, long long> res[2000010];
long long t, n, cnt;
inline bool judge(long long n, long long m, int p) {
	for(int i = 1; i <= p; i++) {
		if(n % m) return 0;
		n /= m++;
	}
...
inline bool comp(long long n, long long m, int p) {
	for(int i = 1; i <= p; i++)
		n /= m++;
	return n;
}
inline void check(int delta) {
	long long m = (long long)(powl(n, 1.0 / delta)) + 1;
	while(m > 1) {
		if(judge(n, m, delta)) {
			res[++cnt] = {m + delta - 1, m - 1};
			return;
		}
		if(comp(n, m, delta)) return;
		m--;
	}
}
int main() {
	...
	while(t--) {
		...
		if(n == 1)    io.writeln(-1);
			...
			res[++cnt] = {n, n - 1};
			for(int i = 2; i <= 20; i++)    check(i);
			io.writeln(cnt);
			for(int i = cnt; i >= 1; i--)    io.writeln(res[i].first, res[i].second);
		}
	}
	//system("pause");
	return 0;
