...
int main() {
	...
	for(int i = 1; i <= n; i++)
		for(int j = 0; j <= i; j++)
			for(int k = 0; k <= m; k++) {
				auto lst = dp[i - 1][j][k];
				dp[i][j][min(k + i, m)] = (dp[i][j][min(k + i, m)] + ((2 * j + 1) * lst) % mod) % mod;
				dp[i][j + 1][k] = (dp[i][j + 1][k] + lst) % mod;
				if(j) dp[i][j - 1][min(m, k + i * 2)] = (dp[i][j - 1][min(m, k + i * 2)] + (lst * j * j) % mod) % mod;
			}
	res = dp[n][0][m];
