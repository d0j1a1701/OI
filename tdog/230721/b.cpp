...
long long n, k, arr[20], res;
// x = a + b * c
// /: c->10c+arr[i]
// +: a->a+bc,b=1,c=arr[i]
// -: a->a+bc,b=-1,c=arr[i]
// *: b->bc,c=arr[i]
void dfs(int i, long long a, long long b, long long c) {
	if(i == n)    return res += ((a + b * c) == k), void();
	dfs(i + 1, a, b, c * 10 + arr[i + 1]);
	dfs(i + 1, a + b * c, 1, arr[i + 1]);
	dfs(i + 1, a + b * c, -1, arr[i + 1]);
	dfs(i + 1, a, b * c, arr[i + 1]);
}
int main() {
	io.read(n, k);
	for(int i = 1; i <= n; i++)   io.read(arr[i]);
	dfs(1, 0, 1, arr[1]);// arr[1] 前面不能填
	io.writeln(res);
	system("pause");
	return 0;
}