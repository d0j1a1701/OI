...
// A-Z 分别为 n+1 到 n+26
int n, cnt[5000030], deg[5000030];
list<int> edges[5000030];
queue<int> q;
...
void topo() {
	...
	while(q.size()) {
		int u = q.front();
		q.pop();
		auto x = cnt[u] * qpow(edges[u].size(), mod - 2) % mod;
		for(int v : edges[u]) {
			cnt[v] = (cnt[v] + x) % mod;
			if(--deg[v] == 0)
				q.emplace(v);
...
int main() {
	...
	for(int i = 1; i <= n; i++) {
		...
		for(int j = 1; j <= m; j++) {
			string buf;
			io.read(buf);
			if(isalpha(buf[0])) edges[i].emplace_back(n + 1 + buf[0] - 'A'), deg[n + 1 + buf[0] - 'A']++;
			else {
				int x = 0;
				sscanf(buf.c_str(), "%d", &x);
				edges[i].emplace_back(x), deg[x]++;
			}
		}
	}
	topo();
