...
int n, m, cnt[10], bas[10], ful[10], now[10];
LL f[60010][100];
string s;
int main() {
	io.read(s, m);
	...
	for (int i = 1; i <= 9; i++) {
		bas[i] = ful[i - 1] + 1;
		ful[i] = ful[i - 1] + bas[i] * cnt[i];
	}
	for (int i = 1; i <= 9; i++)
		if (cnt[i]) f[bas[i]][i % m] = 1;
	for (int s = 1; s <= ful[9]; s++) {
		int ss = s;
		for (int i = 9; i >= 0; i--)
			if (cnt[i]) now[i] = cnt[i] - ss / bas[i], ss %= bas[i];
		for (int i = 0; i <= 9; i++) {
			if (!now[i]) continue;
			for (int k = 0; k < m; k++) {
				int kk = (k * 10 % m + i % m) % m;
				f[s + bas[i]][kk] = (f[s + bas[i]][kk] + f[s][k]) % Mod;
			}
		}
	}
	io.writeln(f[ful[9]][0]);
	system("pause");
	return 0;
}
