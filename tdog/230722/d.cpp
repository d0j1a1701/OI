...
long long primes[1000010], len;
bitset<1000010> mark;
void eratosthenes() {
	mark.reset();
	for(long long i = 2; i * i <= 1000000; i++) {
		if(!mark[i]) for(long long j = i << 1; j <= 1000000; j += i)   mark[j] = 1;
	}
	for(int i = 2; i <= 1000000; i++) if(!mark[i]) primes[++len] = i;
}
long long a, b, d, fac[1000010], cnt;
...
inline long long sqrsum(long long x) {
	return x * (x + 1) % mod * (2 * x + 1) % mod * qpow(6, mod - 2) % mod;
}
long long calc(long long i) {
	long long size = (d + 2) / i;
	long long ans = sqrsum(size) * i % mod * i % mod;
	return ans;
}
long long res;
void dfs(long long tot, int x, int now) {
	if (now > cnt) {
		if (x % 2)  res -= calc(tot);
		else res += calc(tot);
		res = (res % mod + mod) % mod;
...
int main() {
	...
	long long dis = (a * a % mod + b * b % mod) % mod;
	solve(d + 2);
	dfs(1, 0, 1);
	io.writeln(dis * res % mod);
	system("pause");
	return 0;
}
