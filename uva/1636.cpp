...
#include <cstring>
using namespace std;
/* d0j1a_1701 FastIO full ver. 3.7 */
//#define FIO //fread 模式 将不能使用键盘读入 请勿在交互题打开
struct IO {
#ifdef FIO
	...
	string sep = " ";
	int k = 2;
	template<typename Tp, typename enable_if<is_integral<Tp>::value>::type * = nullptr >inline int read(Tp &s) {
		int f = 1;
		char ch = getchar();
		s = 0;
...
int main() {
	while(io.read(arr + 1)) {
		int n = strlen(arr + 1);
		int cnt0 = 0, cnt01 = 0, cnt1 = 0;
		for(int i = 1; i <= n; i++)	cnt0 += arr[i] == '0';
		for(int i = 1; i < n; i++)	cnt01 += arr[i] == '0' && arr[i + 1] == '1';
		cnt01 += arr[n] == '0' && arr[1] == '1';
		cnt1 = n - cnt0;
		int a = cnt01 * n, b = cnt0 * cnt1;
		if(a > b)	io.writeln("ROTATE");
		else if(a == b)	io.writeln("EQUAL");
		else io.writeln("SHOOT");
	}
	return 0;
}