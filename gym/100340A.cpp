...
long long n, m, res;
inline long long query(long long i, long long x) {
	int sum = 0;
	for(int j = 1; j <= n; j++)
		if(i != j)
			sum += arr[j].cnt > x;
...
int main() {
	...
	io.read(n, m);
	for(int i = 1; i <= n; i++)   arr[i] = {io.read(), i, 0};
	sort(arr + 1, arr + 1 + n, [](auto a, auto b) {
		return a.g > b.g;
	});

	for(int i = 1; i <= n; i++)
		res += query(i, arr[i].cnt) * arr[i].g;
	sort(arr + 1, arr + 1 + n, [](auto a, auto b) {
		return a.id < b.id;
	});
	io.writeln(res);
	for(int i = 1; i <= n; i++)   io.writeWith(arr[i].cnt, " ");
	io.writeln();
	//system("pause");
	return 0;
}