...
int t, n, arr[50010], tree[50010];
inline int lowbit(int x) {
	return x & (-x);
}
inline void adjust(int pos, int x) {
	for(; pos <= 50000; pos += lowbit(pos))   tree[pos] += x;
}
...
int main() {
	io.read(t);
	for(int _ = 1; _ <= t; _++) {
		io.write("Case", _).writeln(":");
		memset(tree, 0, sizeof(tree));
		io.read(n);
		for(int i = 1; i <= n; i++)   adjust(i, io.read());
		string op;
		int i, j;
		while(io.read(op) && op != "End") {
			io.read(i, j);
			if(op == "Add")   adjust(i, j);
			else if(op == "Sub")  adjust(i, -j);
			else io.writeln(query(j) - query(i - 1));
		}
	}
	//system("pause");
	return 0;
