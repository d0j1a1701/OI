...
int n, m, arr[200010], x[600010], y[600010], nxt[200010], l, r, res;
bool vis[200010];
inline bool check(int k) {
	copy(arr + 1, arr + 1 + 1 + n, nxt + 1);
	fill(vis + 1, vis + 1 + 1 + n, 0);
	for(int i = 1; i <= k; i++)   swap(nxt[x[i]], nxt[y[i]]);
	int cnt = 0;
	for(int i = 1; i <= n; i++)
		if(!vis[i])
			for(int j = nxt[i]; j != i; j = nxt[j])    vis[j] = 1, cnt++;
	return cnt <= k;
}
int main() {
	...
	io.read(n);
	for(int i = 1; i <= n; i++)   arr[i] = io.read() + 1;
	io.read(m);
	for(int i = 1; i <= m; i++)   x[i] = io.read() + 1, y[i] = io.read() + 1;
	l = 0, r = m, res = -1;
	while(l <= r) {
		int mid = (l + r) >> 1;
