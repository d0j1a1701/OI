...
int n, m, cnt, uploader[300010], which[300010], sz[300010], ptr[300010], res;
int main() {
	freopen("clear.in", "r", stdin);
	freopen("clear.out", "w", stdout);
	io.read(n, m);
	for(int i = 1, l = 1, op, x; i <= m; i++) {
		io.read(op, x);
		if(op == 1)    ++cnt, uploader[cnt] = x, which[cnt] = ++sz[x], res++;
		else if(op == 2) {
			res -= sz[x] - ptr[x];
			ptr[x] = sz[x];
		} else {
			for(; l <= x; l++)
				if(ptr[uploader[l]] < which[l])
					ptr[uploader[l]]++, res--;
		}
		io.writeln(res);
	}
