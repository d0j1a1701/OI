...
long long tid, n, m, arr[500010], nxt[500010], pre[500010];
bool del[500010];
int main() {
	...
	pre[1] = n, nxt[n] = 1;
	for(int i = 1; i <= n; i++)   pq.emplace(arr[i] + arr[nxt[i]], i);
	m = n - m;
	while(m--) {
		int i = pq.top().second;
		pq.pop();
		if(del[i] || del[nxt[i]]) continue;
		if(arr[nxt[i]] > arr[i])  i = nxt[i];
		del[i] = 1;
		pq.emplace(arr[pre[i]] + arr[nxt[i]], pre[i]);
		nxt[pre[i]] = nxt[i];
		pre[nxt[i]] = pre[i];
	}
	long long res = 0;
	while(pq.size()) {
		auto pi = pq.top();
		pq.pop();
		io.writeln(pi.first, pi.second);
		if(del[pi.second] || del[nxt[pi.second]]) continue;
		res = pi.first;
		//break;
	}
	io.writeln(pq.top().first);
	//system("pause");
	return 0;
}
/*
