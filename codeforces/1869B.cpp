...
long long t, n, k, a, b, x[200010], y[200010];
inline auto getDistance(int i, int j) {
...
int main() {
	...
	while(t--) {
		...
		for(int i = 1; i <= k; i++) A = min(A, getDistance(a, i));
		for(int i = 1; i <= k; i++) B = min(B, getDistance(b, i));
		io.writeln(min(A + B, getDistance(a, b)));
	}
	//system("pause");
	return 0;
}