...
long long n, m, dist[510][510], res;
list<Edge> edges;
int t;
int main() {
	io.read(t);
	while(t--) {
		...
		for(auto [u, v, w] : edges) {
			auto cur = min(dist[1][u] + dist[v][n], dist[1][v] + dist[u][n]);
			for(int j = 1; j <= n; j++)
				cur = min(cur, min(dist[u][j] + 1 + dist[1][j] + dist[j][n], dist[v][j] + 1 + dist[1][j] + dist[j][n]));
			res = min(res, w * (cur + 1));
		}
		io.writeln(res);
	}
