...
long long n, m, d, res, tag, pre;
priority_queue<long long>L;
priority_queue<long long, vector<long long>, greater<long long> >R;
signed main() {
	io.read(n, m, d);
	for(long long i = 1, a, b, t; i <= m; ++i) {
		io.read(a, b, t);
		res += b;
		if(i == 1)L.push(a), R.push(a), pre = t;
		else {
			tag += d * (t - pre);
			long long l = L.top() - tag, r = R.top() + tag;
			if(a < l)L.pop(), L.push(a + tag), L.push(a + tag), R.push(l - tag), res -= l - a;
			else if(a > r)R.pop(), R.push(a - tag), R.push(a - tag), L.push(r + tag), res -= a - r;
