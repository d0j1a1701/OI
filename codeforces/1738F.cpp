...
int n, deg[1010], col[1010];
vector<int> edges[1010];
set<pair<int, int> > s;
void dfs(int id, int c) {
	if(col[id]) return;
	col[id] = c;
	for(int it : edges[id]) dfs(it, c);
}
void solve() {
	io.read(n);
	s.clear();
	for(int i = 1; i <= n; i++) edges[i].clear(), col[i] = 0;
	for(int i = 1; i <= n; i++) s.insert({deg[i] = io.read(), i});
	while(!s.empty()) {
		auto t = *--s.end();
		s.erase(--s.end());
		...
		for(int j = 0; j < d; j++) {
			io.writeln('?', id).flush();
			int res = io.read();
			edges[id].push_back(res);
			edges[res].push_back(id);
	...
	io.writeWith('!');
	for(int i = 1; i <= n; i++) io.writeWith(col[i]);
	io.writeln().flush();
}
int main() {
	int t = io.read();
