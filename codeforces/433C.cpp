...
long long n, m, sum, res, arr[100010];
int main() {
	io.read(n, m);
	for (int i = 1; i <= m; i++) io.read(arr[i]);
	for(int i = 2; i <= m; i++) {
		sum += abs(arr[i] - arr[i - 1]);
		if (arr[i - 1] != arr[i]) f[arr[i]].push_back(arr[i - 1]);
	}
	for (int i = 1; i < m; i++)
		if (arr[i + 1] != arr[i]) f[arr[i]].push_back(arr[i + 1]);
	res = sum;
	for (int i = 1; i <= n; i++) {
		...
		long long k = f[i][f[i].size() / 2], s1 = 0, s2 = 0;
		for (int j = 0; j < (int)f[i].size(); j++) s1 += abs(f[i][j] - i), s2 += abs(f[i][j] - k);
		res = min(res, sum - s1 + s2);
	}
	io.writeln(res);
	//system("pause");
	return 0;
