...
/*
对于每个 (x,y):
    1. 若该列未凑齐 [0,k-1] 如果该单元能选取缺少的数，则选缺少的最小的，否则选能选取的最大的
    2. 若该列已凑齐 [0,k-1] 则选能选取的最大的
*/
int main() {
	io.read(t);
	while(t--) {
...
/*
构造一个矩阵，矩阵的每一行是一个 [0,m-1] 的排列
矩阵的每一列取 MEX 得到一个数列，数列的 MEX 为美丽值，要求美丽值最大

其中 MEX 是 [0,n-1] 中第一个不包含在数组里的数
*/