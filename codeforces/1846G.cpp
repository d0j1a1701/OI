...
int t, n, m, dp[MASK + 10], a[1010], b[1010], d[1010];
inline int readBinary() {
	string buf = io.read<string>();
	int res = 0;
	for(int c = 1, i = 0; i < buf.size(); i++)
		res += (buf[i] - '0') * c, c *= 2;
	return res;
}
int main() {
	...
	while(t--) {
		...
		io.read(n, m);
		int beg = readBinary(), mask = (1 << n) - 1;
		dp[beg] = 0;
		for(int i = 1; i <= m; i++) {
			d[i] = io.read(), a[i] = ~readBinary(), b[i] = readBinary();
			for(int j = 0; j <= mask; j++)
				dp[j & a[i] | b[i]] = min(dp[j & a[i] | b[i]], dp[j] + d[i]);
		}
		for(int i = 1; i <= m; i++)
			for(int j = 0; j <= mask; j++)
				dp[j & a[i] | b[i]] = min(dp[j & a[i] | b[i]], dp[j] + d[i]);
