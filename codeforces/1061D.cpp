...
int main() {
	...
	stack<long long> st;
	long long res = 0;
	sort(arr + 1, arr + n + n + 1, [](auto x, auto y) {
		return x.t == y.t ? x.d < y.d : x.t < y.t;
	});
	for (int i = 1; i <= n + n; i++) {
		if (arr[i].d) st.push(arr[i].t);
		else {
			auto y = a[arr[i].id].second;
			if (st.empty()) (res += (X + Y * (y - arr[i].t) % mod) % mod) %= mod;
			else {
				auto x = st.top();
				if (Y * (y - x) < X + Y * (y - arr[i].t)) (res += Y * (y - x) % mod) %= mod, st.pop();
				else (res += (X + Y * (y - arr[i].t) % mod) % mod) %= mod;
			}
		}
	}
	io.writeln(res);
