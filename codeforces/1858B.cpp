...
long long s[100010], t, n, m, d, sum, res, mark, cnt;
inline long long calc(int i, int j) {
	return (s[j] - s[i] + d - 1) / d;// 避免浮点误差
}
int main() {
	io.read(t);
	while(t--) {
		sum = res = mark = cnt = 0;
		io.read(n, m, d);
		for(int i = 1; i <= m; i++)   io.read(s[i]);
		if(s[1] != 1) {
			for(int i = m + 1; i > 1; i--) s[i] = s[i - 1];
			s[1] = mark = 1, m++;
		}
		s[++m] = n + 1;
		for(int i = 1; i < m; i++)   sum += calc(i, i + 1);
		res = sum;
		for(int i = 2; i < m; i++)    res = min(res, sum - calc(i - 1, i) - calc(i, i + 1) + calc(i - 1, i + 1));
		for(int i = 2; i < m; i++)    cnt += (sum - calc(i - 1, i) - calc(i, i + 1) + calc(i - 1, i + 1)) == res;
		if (sum < res && !mark) res = sum, cnt = 1;
		else if (sum == res && !mark) res = sum, cnt++;
		io.writeln(res, cnt);
	}
	//system("pause");
	return 0;
