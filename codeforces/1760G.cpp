...
int t, n, a, b;
set<int> s;
void dfs1(int id, int fa, int x) {
	for(auto it : edges[id])
		if(it.first != fa)
			s.emplace(x ^ it.second),
			          dfs1(it.first, id, x ^ it.second);
}
bool dfs2(int id, int fa, int x) {
	if((id == b && x == 0) || s.count(x))  return true;
	bool res = false;
	for(auto it : edges[id])
		if(it.first != fa && !(it.first == b && (x ^ it.second) != 0))
			res |= dfs2(it.first, id, x ^ it.second);
	return res;
}
int main() {
	io.read(t);
