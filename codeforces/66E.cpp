...
int n, a[500010], b[500010], res[500010], sum1[500010], q1[500010], l1, r1, sum2[500010], q2[500010], l2, r2;
inline bool A(int x) {
	int sum = -sum1[x - 1];
...
int main() {
	...
	for (int i = 1; i <= n * 2; i++) sum1[i] = sum1[i - 1] + a[i] - b[i];
	for (int i = 1; i <= n * 2; i++) sum2[i] = sum2[i - 1] + a[i] - b[i - 1];
	l1 = l2 = 1;
	for (int i = 1; i < n; i++) {
	...
	for (int i = 1; i <= n; i++) {
		if(A(i) || B(i)) res[++res[0]] = i;
	}
	io.writeln(res[0]);
	for (int i = 1; i <= res[0]; i++) io.writeWith(res[i]);
	io.writeln();
	return 0;
}
