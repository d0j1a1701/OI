...
long long t, n, c, arr[200010], pfx[200010], res;
map<long long, long long> mp;
inline long long solve(long long m, long long x, long long id) {
	long long l = 1, r = n, sum = 0;
	while(l <= r) {
		auto mid = (l + r) >> 1;
		if(pfx[mid] - (mid >= id ? x : 0) <= m)
...
int main() {
	...
	while(t--) {
		...
		for(int i = 1; i <= n; i++)   pfx[i] = min(arr[i] + (n - i + 1), arr[i] + i);
		sort(pfx + 1, pfx + 1 + n);
		for(int i = 1; i <= n; i++)   mp[pfx[i]] = i, pfx[i] += pfx[i - 1];
		for(int i = 1; i <= n; i++) {
			if(c - arr[i] - i < 0)    continue;
			res = max(res, solve(c - arr[i] - i, min(arr[i] + (n - i + 1), arr[i] + i), mp[min(arr[i] + (n - i + 1), arr[i] + i)]) + 1);
		}
		io.writeln(res);
	}
	//system("pause");
