...
int n, k, p, x, y, arr[1010];
int main() {
	io.read(n, k, p, x, y);
	for(int i = 1; i <= k; i++) io.read(arr[i]);
	int now = count_if(arr + 1, arr + 1 + k, [](int x) {
		return x >= y;
	});
	if(now + n - k < (n + 1) / 2) {
		io.writeln(-1);
	...
	int res = max(0, (n + 1) / 2 - now);
	for(int i = k + 1; i <= k + res; i++)
		arr[i] = y;
	for(int i = k + res + 1; i <= n; i++)
		arr[i] = 1;
	int sum = accumulate(arr + 1, arr + 1 + n, 0);
	if(sum > x) io.writeln(-1);
	else
		for(int i = k + 1; i <= n; i++)
			io.writeWith(arr[i]);
	io.writeln();
