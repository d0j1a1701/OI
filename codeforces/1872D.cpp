...
long long t, n, x, y;
inline auto sum(auto a, auto l, auto d) {
	return (a + a + d * (l - 1)) * l / 2;
}
int main() {
	io.read(t);
	while(t--) {
		io.read(n, x, y);
		auto cntx = n / x, cnty = n / y;
		auto cntxy = n / lcm(x, y);
		io.writeln(sum(n, cntx, -1) - sum(1, cnty, 1) + sum(cnty, cntxy, -1) - sum(n - cntx + 1, cntxy, 1));
	}
	//system("pause");
	return 0;
}
