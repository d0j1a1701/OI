...
int n, arr[200010], lst[200010], dp[200010][3];
int main() {
	io.read(n);
	memset(dp, 0x3f, sizeof(dp));
	for(int i = 1; i <= n; i++)   io.read(arr[i]);
	for(int i = 1; i <= n; i++)   lst[i] = arr[i] ? lst[i - 1] : i;
	dp[1][arr[1]] = 0;
	for(int i = 2; i <= n; i++) {
		if(arr[i])  dp[i][arr[i] - 1] = dp[lst[i] - 1][0] + 1;
		dp[i][arr[i]] = min(min(dp[i - 1][0] + 1, dp[i - 1][1]), dp[i - 1][2]);
	}
	io.writeln(dp[n][arr[n]]);
	//system("pause");
	return 0;
}