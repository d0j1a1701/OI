...
long long n, arr[200010], f[200010], rot[200010], lst[200010], cnt, res;
...
int main() {
	...
	for(int i = 1, x; x = io.read(), i <= n; i++)
		if(x > 0) edges[x].emplace_back(i);
		else lst[++cnt] = i;
	for(int i = 1; i <= cnt; i++)   dfs(lst[i]);
	io.writeln(res);
	for(int i = 1; i <= cnt; i++)   out(lst[i]);
	io.writeln();
	//system("pause");
	return 0;
}