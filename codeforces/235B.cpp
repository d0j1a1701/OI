...
#include <cstring>
using namespace std;
/* d0j1a_1701 FastIO full ver. 4.0 */
...
double arr[100010], f[100010], res[100010];
int n;
int main() {
	io.read(n);
	for(int i = 1; i <= n; i++)   io.read(arr[i]);
	for(int i = 1; i <= n; i++)   f[i] = (f[i - 1] + 1) * arr[i];
	for(int i = 1; i <= n; i++)   res[i] = res[i - 1] + arr[i] * (2 * f[i - 1] + 1);
	io.setprecision(7).writeln(res[n]);
	//system("pause");
	return 0;
}