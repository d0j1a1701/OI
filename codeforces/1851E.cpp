...
int t, n, k, c[200010], p[200010], deg[200010];
long long res[200010], sum[200010];
list<int> edges[200010];
inline void solve() {
	copy(c + 1, c + 1 + n, res + 1);
	for(int i = 1; i <= k; i++)   res[p[i]] = 0;
	queue<int> q;
	for(int i = 1; i <= n; i++)
	...
	while(q.size()) {
		...
		for(int v : edges[u]) {
			sum[v] += res[u];
			if(--deg[v] == 0)
				q.emplace(v), res[v] = min(res[v], sum[v]);
		}
	}
}
int main() {
	...
	while(t--) {
		...
		for(int i = 1; i <= k; i++)   io.read(p[i]);
		for(int i = 1; i <= n; i++) {
			io.read(deg[i]);
			for(int j = 1; j <= deg[i]; j++)
				edges[io.read()].emplace_back(i);
		}
		solve();
		for(int i = 1; i <= n; i++)   io.writeWith(res[i], " ");
		io.writeln();
	}
	//system("pause");
	return 0;
