...
int t, n, arr[100010], tag[400010], tree[400010][2], q;
char bin[100010];
...
inline void pushdown(int id) {
	if(tag[id]) {
		tag[id << 1] ^= 1;
		tag[id << 1 | 1] ^= 1;
		swap(tree[id << 1][0], tree[id << 1][1]);
		swap(tree[id << 1 | 1][0], tree[id << 1 | 1][1]);
		tag[id] = 0;
	}
}
void build(int id, int l, int r) {
	tag[id] = 0;
	if(l == r)    return tree[id][0] = tree[id][1] = 0, tree[id][bin[l] - '0'] = arr[l], void();
	int mid = (l + r) / 2;
	build(id << 1, l, mid), build(id << 1 | 1, mid + 1, r);
	pushup(id);
}
void adjust(int id, int l, int r, int x, int y) {
	if(x <= l && r <= y) {
		tag[id] ^= 1;
	...
	pushdown(id);
	int mid = (l + r) / 2;
	if(x <= mid)    adjust(id << 1, l, mid, x, y);
	if(mid < y)    adjust(id << 1 | 1, mid + 1, r, x, y);
	pushup(id);
}
int main() {
	io.read(t);
	while(t--) {
		...
		for(int i = 1, op, x, y; i <= q; i++) {
			io.read(op, x);
			if(op == 1)   io.read(y), adjust(1, 1, n, x, y);
			else io.writeWith(tree[1][x]);
		}
		io.writeln();
	}
	//system("pause");
