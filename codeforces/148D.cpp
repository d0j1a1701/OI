...
double mem[1010][1010];
bool valid[1010][1010];
double dfs(int w, int b) {
	if(w < 0 || b < 0) return 0; // 不存在该情况
	if(w == 0)    return 0;
	if(b == 0)    return 1;
	if(valid[w][b]) return mem[w][b];
	double res = 0;
	res = 1.0 * w / (w + b); // A 抓到 白色
	// A 抓到 黑色 | B 抓到 白色 -> 赢不了
	if(b == 1)  goto nxt; // B 无黑可抓
	// A 抓到 黑色 | B 抓到 黑色 | 跑了一只白色
	res += 1.0 * b / (w + b) * (b - 1) / (w + b - 1) * w / (w + b - 2) * dfs(w - 1, b - 2);
	// A 抓到 黑色 | B 抓到 黑色 | 跑了一只黑色
	res += 1.0 * b / (w + b) * (b - 1) / (w + b - 1) * (b - 2) / (w + b - 2) * dfs(w, b - 3);
nxt:
	valid[w][b] = 1;
	return mem[w][b] = res;
}
...
int main() {
	io.read(w, b);
	//printf("%.9lf\n", dfs(w, b));
	io.setprecision(9).writeln(dfs(w, b));
	//system("pause");
	return 0;
}