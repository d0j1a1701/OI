...
struct IO {
	...
	template<typename Tp, typename enable_if<is_floating_point<Tp>::value>::type * = nullptr>inline IO & write(Tp x) {
		...
		} else if (f == 0.5 && ((frac == 0U) || (frac & 1U))) ++frac;
		write(whole);
		if (k == 0U) {
			f = x - (double)whole;
			if ((!(f < 0.5) || (f > 0.5)) && (whole & 1))
				++whole;
		} else {
			putchar('.');
			static char sta[20];
...
int n, m, lg[100010], point, fa[100005][20], dep[100005], deg[100005];
pair<int, int> el[100010];
...
int main() {
	...
	for(int i = 1; i < n; i++) {
		auto [u, v] = el[i];
		if(dep[u] > dep[v]) io.writeWith(deg[u]);
		else io.writeWith(deg[v]);
	}
	io.writeln();
	return 0;
}