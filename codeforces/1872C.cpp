...
int t, l, r, tag[maxn], nxt[maxn], primes[maxn], cnt;
inline void euler() {
	for(int i = 2; i <= 10000000; i++) {
		if(!tag[i]) primes[++cnt] = i;
		for(int j = 1; j <= cnt && i * primes[j] <= 10000000; j++) {
			tag[i * primes[j]] = i;
...
int main() {
	...
	while(t--) {
		io.read(l, r);
		if(nxt[l] <= r) {
			int a = tag[nxt[l]], b = nxt[l] / tag[nxt[l]];
			if(a <= 1 || b <= 1)  io.writeln(-1);
			else io.writeln((a - 1)*b, b);
		} else io.writeln(-1);
	}
	//system("pause");
	return 0;
...
/*
如果 [l,r] 中存在一个可表示为 ab (a,b > 1) 的数则有解
如果 [l,r] 中存在一个合数则有解
*/