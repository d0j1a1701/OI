...
int res, cnt[1000010], ans[1000010], v[1000010], val[1000010], f[1000010][30], pos[1000010];
int m, len, p, i, k;
list<int> e[1000010];
char op[1000010];
...
signed main() {
	...
	for(int i = 1; i <= m; i++) {
		...
		if(op[i] == '+') {
			io.read(val[i]);
			f[++len][0] = p;
			for(int k = 1; k <= 21; k++) f[len][k] = f[f[len][k - 1]][k - 1];
			p = len;
			v[len] = val[i];
			q.push(i);
		} else if(op[i] == '-') {
			io.read(val[i]);
			for(int k = 0; k <= 21; k++) if(val[i] & (1 << k)) p = f[p][k];
			q.push(i);
		} else if(op[i] == '!') p = pos[q.top() - 1], q.pop();
		pos[i] = p;
	}
	for(int i = 1; i <= len; i++) e[f[i][0]].emplace_back(i);
	dfs(0);
	for(int i = 1; i <= m; i++) if(op[i] == '?') io.writeln(ans[pos[i]]);
	return 0;
}